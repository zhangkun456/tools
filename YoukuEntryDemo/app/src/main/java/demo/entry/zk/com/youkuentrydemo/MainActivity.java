...
//            }
    ...
    private void openNativePage(String url) {
        try {
            ...
            String param = "https://weex.alibabaplanet.com/wow/planet/act/entry-demo?wh_weex=true"
                    + "&navigatorurl=" + toHexString(URLEncoder.encode(url));

            param = URLEncoder.encode(param);
            System.out.println("openNativePage =" + param);
            intent.setData(Uri.parse("youku://planet/weex?_wx_tpl=" + param));
            startActivity(intent);
        } catch (Exception e) {
            e.printStackTrace();
        }
